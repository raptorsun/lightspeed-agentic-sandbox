...
class Resolver:
    ...
    def resolve(  # pylint: disable=too-many-branches
...
def _generate_hermetic_requirements(
    ...
    """Generate requirements.hermetic.txt from bootstrap_packages via RHOAI index."""
    ...
    for name in bootstrap_packages:
        result = rhoai.find_best(name, ">=0")
        if not result:
            logger.warning("Bootstrap package %s not found on RHOAI index", name)
            continue
        pkgs[normalize_name(name)] = {
            "version": result["version"],
            "platforms": result["platforms"],
        }
    write_hashed_requirements(pkgs, hermetic_path, rhoai_index_url)
    logger.info("Wrote %s (%d bootstrap packages)", hermetic_path, len(pkgs))
...
def _run_uv_compile(
    ...
    for raw_line in result.stdout.splitlines():
        ...
        if m:
            ...
            version = (
                version_spec[2:] if version_spec.startswith("==") else version_spec.lstrip("=")
            )
            if version:
...
def uv_resolve(
    ...
    initial = _run_uv_compile(python_version, [manual_overrides], extras=extras)
    ...
    if count > 0:
        ...
        resolved = _run_uv_compile(
            python_version, [manual_overrides, auto_overrides], extras=extras
        )
    else:
        resolved = initial
    ...
    for pkg in [n for n in resolved if n.startswith(cuda_prefixes) and not rhoai.has_package(n)]:
...
def _strip_rhoai_duplicates_from_build_deps(build_file: str, rhoai_names: set[str]) -> None:
...
def main() -> None:
    ...
    resolved_versions, rhoai = uv_resolve(
        python_version, rhoai_index_url, suffix, platforms, extras=extras
    )
    ...
    logger.info("Classified: %d RHOAI, %d PyPI", rhoai_count, len(resolved) - rhoai_count)
    ...
    _generate_hermetic_requirements(bootstrap_packages, rhoai, rhoai_index_url, suffix)
